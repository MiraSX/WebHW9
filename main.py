...
def create_jsons():
    ...
    for link in next_page():
        ...
        for el in content_quote:
            ...
            born_date = (
                content_bio.find("p")
                .find("span", attrs={"class": "author-born-date"})
                .text
            )
            born_location = (
                content_bio.find("p")
                .find("span", attrs={"class": "author-born-location"})
                .text
            )
            description = content_bio.find(
    ...
    with open("json/authors.json", "w", encoding="utf-8") as fd:
        json.dump(result_author, fd, ensure_ascii=False, indent=2)
